...
class T60_ERB_Extractor(FrontEndFeatureExtraction):
    """Implements a (hopefully) matching version of the front-end feature extraction used from reference paper.

       Gamper and Tashev, "Blind Reverberation Time Estimation Using a convolutional Neural Network," 2018.

    """
    def __init__(self, sample_rate, require_normalization, output_len=1996):
        super().__init__(sample_rate)
        self.require_normalization = require_normalization
        self.spectral_median = 0
        self.mean = 0
    ...
    def extract(self, x):
        ...
        y = va_filterbank.gammatone_filterbank_16k_400_6000Hz_21(x[:, 0])

        # Compute the energy of the frames
        for i in range(y.shape[0]):
            result = librosa.util.frame(y[i, :], frame_length=self.frame_length, hop_length=self.hop_length)
            if i == 0:
                z = np.zeros((y.shape[0], result.shape[1]))
            z[i, :] = librosa.power_to_db(np.sum(np.square(result), axis=0))

        # Take the last 1996 frames
        z = z[:, -out_shape[1]:]

        if self.normalization_set:
            z = z - self.spectral_median
            z = (z - self.mean) / self.std
        features = np.expand_dims(z, axis=2)
